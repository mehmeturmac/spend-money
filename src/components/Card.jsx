...
function Card(product) {
  ...
  return (
    <Box p={4} bg="white">
      ...
      <Center mt={5} bg="none">
        <Button colorScheme="red" mr={3} w="30%" onClick={() => setCount(count - 1)} isDisabled={isSellable}>
          Sell
        </Button>
        <Input type="number" textAlign="center" w="30%" value={count} onChange={(e) => handleInputChange(parseInt(e.target.value))} />
        <Button colorScheme="teal" ml={3} w="30%" onClick={() => setCount(count + 1)} isDisabled={isBuyable}>
          Buy
        </Button>
      </Center>
    </Box>
